...
- Main regions must be clearly defined using a header comment that follows this format:
  ...
  - A divider line made up of 80 equal signs (=) must separate each main region. This divider visually distinguishes one region from the next.
...
- Sub-regions are used to further segment code within a main region.
  ...
  - Add a DESCRIPTION section and, if needed, an IMPORTANT NOTES section to include lessons learned or critical points to avoid recurring errors.
...
- Consistency is key: this structure should be applied across the entire codebase to ensure a standard approach to coding and maintenance.
