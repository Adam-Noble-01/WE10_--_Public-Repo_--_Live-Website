...
| SC30 | **Still Water is its own FILL tag: the Waterbodies wash with no hatch.** Adam, for a pool: "Use the water texture we've already used, but just without the hatching." `74__SitePlan__ExternalWorks__StillWater` fills in MAT802 (the same blue, no new material), `FillHatchId` null, fill Z 4 (level with Waterbodies), and draws any edges exactly like Waterbodies (blue, 1.0 pt, line Z 7). The ripples stay on Waterbodies for OS ponds and lakes. A separate tag because the wash and the hatch are properties of the TAG (SC28). A pool is normally a face tagged alone inside the terrace, so it relies on SC27 | DECIDED (22-Sep-2026) |
| SC31 | **`74__SitePlan__ExternalWorks__HardSurfaces__Removed`** - Adam's name, verbatim - for "paths, roads, etc., that are being removed from within the site". A mid grey L60 **dotted** line (TrueVision's `dotted`: 0.4 mm dots, 1.2 mm gaps), 0.75 pt, line Z 2, **no fill**, and SketchUp's `Dot` line style on the tag. Weight and Z are those of Site Paths, which carries RB05's drives, so a removed drive differs from a kept one only in the dots. No fill because a removed surface should show whatever will replace it, which another tag draws | DECIDED (22-Sep-2026) |
| SC32 | **`72__SitePlan__Boundary__WallsAndFences__Removed`** for Adam's "Add fences removed". Named his way (the fences' own tag plus `__Removed`), so it sorts directly under Walls and Fences and covers walls, gates and piers too. **Everything being removed reads alike:** the SC31 mid grey dotted line and `Dot` style, at the fences' 0.75 pt and line Z 3, with no fill. A kept fence stays soft black and solid. Edges are MOVED onto the tag, not traced over, because dots painted over a solid line read as solid | DECIDED / `INTERPRETED` (22-Sep-2026) |
| SC33 | **`73__SitePlan__Buildings__Existing__Removed`** - Adam's name - "a blue, small dotted line showing the removed building, no fill". Blue `MTE205`, `dotted`, **0.5 pt with `SitePlan__LineDashScale` 0.5**: a 0.8 mm repeat, which the painter's ROUND caps draw as 0.38 mm dots with 0.42 mm gaps (the other removal lines repeat every 1.6 mm). SMALL uses the existing per-layer dash scale (P4c), not a new line type, so no TrueVision code changed. Line Z 5, `Dot` in SketchUp. Adam chose the colour himself, so buildings do NOT follow SC32's "removed = mid grey". `Buildings__ToBeDemolished` (soft black dashed) is untouched | DECIDED / `INTERPRETED` (22-Sep-2026) |
...
**Still water and removed hard surfaces (SC30, SC31), 22-Sep-2026:** for a pool, select only its face
inside the terrace and tag it `74__SitePlan__ExternalWorks__StillWater`. The terrace edges stay where
they are. It washes water blue with no ripples. For a path or drive being taken out, tag its edges
`74__SitePlan__ExternalWorks__HardSurfaces__Removed`. It prints mid grey and dotted, and shows dotted
in SketchUp. For a wall or fence coming down, use `72__SitePlan__Boundary__WallsAndFences__Removed` (SC32),
which gives the same line. In both cases MOVE the edges onto the tag rather than tracing over them.
For a building coming down, use `73__SitePlan__Buildings__Existing__Removed` (SC33): blue, small dotted,
no fill.
...
| P8a | **Face-only fill layers (finding F6)** - the face-only half of P8 | ~ | SitePlanExport 1.4.0, Store 1.2.0, build script | 21-Sep-2026, SC27. Three gates moved together: `Na__SitePlan__Write` writes a fill GLB alone and a manifest record with `Layer__LineworkFile: null`; the build script accepts it (a NAMED missing linework file is still a fault); `Na__SpStore__Layer` keeps it and `LoadLayer` skips the linework fetch, takes bounds from the fill, and REJECTS if that fill fails. Proven: build script 7 checks, store 32 (5 new), each calibrated against HEAD where it fails; and **in the app on RB05**, a faces-only record injected into the manifest response (browser only, no file touched) painted its wash from the woodland's real fill GLB with no linework behind it. ~~The Ruby is syntax-checked through SketchUp's interpreter only~~ **22-Sep-2026: the real Ruby has now RUN**, in SketchUp's Ruby DLL against a stand-in model, and wrote a faces-only Still Water layer (P6g) - **`~` until Adam exports a face-only fill in SketchUp** |
| P6g | **Still Water: the water wash without the ripples** | ~ | Tags 2.8.0, Materials 1.7.1 | 22-Sep-2026, SC30. **Proven end to end outside SketchUp, on real code only.** The real `Na__SitePlan__Scan` and `Na__SitePlan__Write` ran in SketchUp's own Ruby 3.2 DLL against a stand-in model: a terrace group on Site Paths, and inside it a pool face tagged Still Water whose edges stay with the terrace. Runners and harness are kept in `D:/_ClaudeScratch/siteplan_proof_runners/`. The exporter wrote a FILL GLB alone: "1 ring(s) from 1 face(s), faces only", manifest `Layer__LineworkFile: null`, fill `#BAE4FD`, no hatch, Z 7/4. The real ProjectVision build script took that folder. The real TrueVision store and GLB parser then loaded one outer ring at the pool's true drawing-mm corners and fetched no linework file. SSOT checks: 104 composites, 32 store. **`~` until Adam tags a pool in SketchUp and exports** |
| P6h | **Hard surfaces to be removed: a mid grey dotted line** | ~ | Tags 2.8.0 | 22-Sep-2026, SC31. The same Ruby run exported 4 segments on it. The real `Na__LeEdge__Effective` resolves the layer to `mid-grey` `#999999`, `dotted` `[0.4, 1.2]` mm, 0.265 mm, with Site Paths beside it staying solid. **Three new guards over EVERY site plan tag:** it is in both exclusion lists (would have caught `OsMapping__MajorFeature`, fixed only in 2.7.1); its line type is a TrueVision alias; its line colour is in the EdgeStyles palette (F4). **`~` until Adam tags a removal in SketchUp and exports** |
| P6i | **Walls and fences to be removed** | ~ | Tags 2.9.0 | 22-Sep-2026, SC32. Same proof chain as P6h. The Ruby exporter wrote 2 removed fence runs at line Z 3 beside a kept fence, and the build script took both. TrueVision's store loaded it in Boundaries, and `Na__LeEdge__Effective` resolves `#999999` dotted 0.265 mm against the kept fence's `#333333` solid. The real Tag Manager's loader lists it, and the one-shot sets `Dot` on both removal tags. 2 new SSOT checks (106 composites) plus the three every-tag guards. **`~` until Adam tags a removed fence in SketchUp and exports** |
| P6j | **Existing buildings to be removed: blue, small dotted** | ~ | Tags 2.9.0 | 22-Sep-2026, SC33. Same proof chain: a shed on the tag beside a barn on Existing Buildings. The manifest carries `LineDashScale: 0.5`, and the build script and store keep it. `Na__LeEdge__Effective` resolves `#1E88E5` `dotted` `[0.2, 0.6]` mm at 0.176 mm with no fill, against the barn's `#E53935` solid 0.529 mm. The Tag Manager lists it and the one-shot sets `Dot`. 2 new SSOT checks (108 composites). **`~` until Adam tags a removed building in SketchUp and exports, and sees whether "small" is small enough** |
