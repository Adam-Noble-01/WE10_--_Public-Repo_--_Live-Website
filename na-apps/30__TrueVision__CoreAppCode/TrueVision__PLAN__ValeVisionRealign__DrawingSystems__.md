...
| M | Tested by Adam | - | 2.29.0 | Awaiting sign-off before the ValeVision port |
| M | Gradient back-port to ValeVision | - | - | Pending Adam's sign-off. The module and its config port verbatim; the wiring points are the Vectors panel, `PushPolyline` and both polyline painters in the sheet chrome, shape geometry `Push` and `Hit`, the shape normaliser, `CreateShape` / `UpdateShape`, the shape defaults, the draw tool, the eyedropper trait, the mode controller's Ready and the panel stylesheet |
| - | Parity ledger updated | x | - | Both directions recorded; ValeVision's ledger carries the return-trip table |
| **J** | **Per-category projected edge styles** | ~ | 2.27.0 | Awaiting Adam's re-test after the GPU fix. Colour, line type and weight per SketchUp category per viewport; composite layer weights; Advanced fold UI. See DEVLOG v2.27.0 |
...
| J | Configs | x | 2.27.0 | `EdgeStyles__Config__` (SSOT greys by alias, 7 line types in paper mm), `RenderComposites__Config__` (panel built from it), ModelLayers config 1.1.0 with per-category defaults. `Drawing2dEdgeWidth` moved from the main config to the profileLinework composite weight; DrawView default 1.0 -> 0.55 so the drawing view is unchanged |
...
| J | Tested by Adam | - | 2.27.0 | |
| R | ValeVision port of J | - | - | After sign-off. ValeVision's `auto` has the same GPU route (ported in R) and needs the same kept-render rule, not just the owner tags |
