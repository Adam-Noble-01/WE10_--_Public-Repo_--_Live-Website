...
| **AP** | **Short sheet tabs - the register's D-number, then a short typed name** | **x** | 2.70.0 | Authored in TrueVision first, from Adam's request. A sheet tab reads `D03 - 3D Images` (label `TabLabelFormat`): `Na__LeRec__ShortCode` cuts the last letter-led code off the Drawing Register's number (`PS01_T02_D03` gives `D03`; bare digits or no code give none and the tab shows the name alone), and the whole number moves to the tab's hover. `Sheet__Name` holds words only: `NormaliseSheet` strips a typed code of the pack's own series (`StripSheetCode`), and `CleanSheetName` does the same before a rename is saved. `ApplySheetName` is the one rename and no longer overwrites a Drawing Title that differs from the name. Sheet panel: the code stands in front of the Name box as fixed text, and the Drawing Title row is back. Toolbar, Dev menu and specification chips read `Na__LeModel__GetTabLabel`. New sheets are called `New Drawing`. No new record field. Also: the Drawing Register page's ground matches the Project Specification's (`#eef1f4` page, `#d7dde3` Read desk) |
| AP | Verified | x | 2.70.0 | 37 table cases on the functions lifted from the real module; named exports (339 files) and the module graph pass; twelve modules parse as `.mjs`; config JSON parses with no duplicate keys. In the app on PS01 (localhost:8652, no-cache server, every write refused, none attempted): tabs, hover, panel code, toolbar, rename frame, Drawing Title row, register rows and both grounds read back from the DOM; a simulated renumber to `PS01_T02_D117` moved tab, hover, panel and toolbar and was put back exactly; PS01's four long titles survived the load untouched |
| AP | Tested by Adam | - | 2.70.0 | |
| AP | ValeVision port | - | - | Pending Adam's sign-off, and only the half that stands alone: ValeVision has no Drawing Register, so its sheets carry no register number and every tab would show its name alone. Portable now: `DrawingNumber`, `ShortCode`, `StripSheetCode`, the `GetTabLabel` family, `ApplySheetName` (ValeVision's `UpdateSheet` does not write the title, so there is nothing to undo there), the Name row's fixed code and the tab rename frame. The register ground waits for the register itself |
